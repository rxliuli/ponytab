...
export async function getWallpaperUrl(): Promise<string> {
  const data = await storage.get(STORAGE_KEYS.CURRENT_WALLPAPER)
  const imageUrl = data[STORAGE_KEYS.CURRENT_WALLPAPER]
  if (!imageUrl) {
...
async function shouldUpdateMetadata(duration: number): Promise<boolean> {
  try {
    const data = await storage.get(STORAGE_KEYS.LAST_UPDATE_TIME)
...
function selectRandomWallpaper(metadata: string[]): string {
